define([
	...
	ploneHelper.upload = function(host, username, password, path, title, content, callback) {
		var task = new AsyncTask();
		connect(task);
		task.onRun(function() {
			var url = constants.plone_PUBLISH_URL + '?' + $.param({
				host: host,
				port: 80,
				username: username,
				password: password,
	...
	function handleError(error, task) {
		...
		if(error) {
			...
			if(typeof error === "string") {
				errorMsg = "plone error: " + error + ".";
			}
			else {
				errorMsg = "Could not publish on plone server.";
				if(error.code <= 0) {
					core.setOffline();
